...
# Operational Cities supported by Chatbot
...
# Function for searching a Restaurant on the basis of the parameters
def RestaurantSearch(City, Cuisine, price_max, price_min):
    print(City, Cuisine, price_max, price_min)
    return ZomatoData[
...
def sendmail(mail_id, data):
    sender = "pyarabot@gmail.com"
    receivers = [mail_id]

    message = MIMEMultipart("alternative")
    message["Subject"] = "Top 10 Foodie Restaurants"
    message["From"] = sender
    # HTML for Sending mails
    html = """
		...
		* {
		...
		<table style="font-family:'Montserrat',sans-serif; justify-content: center; display: table-cell" role="presentation" cellpadding="0" cellspacing="0" width="80%" border="2">
		<div>
			<thead>
			<tr>
    ...
    try:
        ...
        smtpObj.starttls()
        smtpObj.login(sender, "Pass@1234")
...
# Action for searching restaurants
class ActionSearchRestaurants(Action):
    def name(self):
        return "action_search_restaurants"

    def run(self, dispatcher, tracker, domain):
        # extracting chatbot data
        loc = tracker.get_slot("location")
        cuisine = tracker.get_slot("cuisine")
        price = tracker.get_slot("price")
        # min and max price on the basis of flag
        if price == "high":
        ...
        if loc.lower() in WeOperate:
            ...
            if results.shape[0] == 0:
                ...
                for restaurant in (
                    ...
                    response = (
                        response
                        + f"Found {restaurant['Restaurant Name']} in {restaurant['Address']}  has been rated {restaurant['Aggregate rating']} with avg cost {restaurant['Average Cost for two']} \n\n"
                    )
                    search_validity = "valid"
        else:
            # validating the results
            response = "Sorry we Can't Operate here !!"
            search_validity = "invalid"
        dispatcher.utter_message("Found data" + response)
        return [SlotSet("location", loc), SlotSet("search_validity", search_validity)]
...
class ActionSendMail(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if results.shape[0] == 0:
            ...
            for restaurant in (
                RestaurantSearch(loc, cuisine, price_max=price_max, price_min=price_min)
                .iloc[:10]
                .iterrows()
            ):
                restaurant = restaurant[1]
                data = (
                    data
                    + f"<tr><td> {restaurant['Restaurant Name']} </td><td> {restaurant['Address']} </td><td> {restaurant['Average Cost for two']} </td><td> {restaurant['Aggregate rating']} </td></tr>"
                )
        print(MailID)
...
# Action for validating the location
...
# Action for restarting the chat
...
# Action for resting all slots values
